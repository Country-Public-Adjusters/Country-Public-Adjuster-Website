// Extracts structured lead information from a Sarah ↔ visitor conversation
// using Claude tool-use. Returns null fields for anything the visitor didn't
// actually say — never invents data.
...
const EXTRACTION_TOOL = {
  name: 'extract_lead',
  description:
    'Extract structured lead information from a conversation between Sarah (an AI intake assistant) and a property owner. Use ONLY information the property owner explicitly stated. Never invent or guess. If a field was not mentioned, return null.',
  input_schema: {
    type: 'object',
    properties: {
      firstName: {
...
Read the conversation between Sarah (assistant) and a property owner (visitor) and call the extract_lead tool with what was actually said.
